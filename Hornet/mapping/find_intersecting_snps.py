...
def get_indels(snp_dict):
    ...
    for chrom in snp_dict:
        for pos, alleles in snp_dict[chrom].items():
            # edit to handles alleles as single string created in get_snps() by "".join([ref,alt]) above,
            # for which the first 2 conditiions don't seem to ever apply...
            if ('-' in alleles) or (max(len(i) for i in alleles) > 1) or ((len(alleles) > 2) and (',' not in alleles)):
...
def get_dual_read_seqs(read1, read2, snp_dict, indel_dict, dispositions,
    ...
    if 'chr' not in read1.reference_name:
        chrom = 'chr' + read1.reference_name
    else:
        ...
        for (read_pos, ref_pos) in read.get_aligned_pairs(matches_only=True):
            ...
            if ref_pos in snp_dict[chrom]:
                ...
                if len(snp_dict[chrom][ref_pos]) > 2:
                    # This happens if the string has a comma in it (indels already dropped)
                    warn("Multiple alleles at {}:{}. Reads across position dropped".format(chrom, ref_pos))
...
def assign_reads(insam, snp_dict, indel_dict, is_paired=True, phased=False):
    ...
    for i, read in iterator:
        ...
        if not is_paired:
            read_seqs = get_read_seqs(read, snp_dict, indel_dict, read_results,
                                     phased)
            remap_num += write_read_seqs([(read, read_seqs)], keep, remap_bam,
    ...
    print("  Reads dropped [multivalent SNPs]:", read_results['toss_multi_allele_snps'], "(" + "%.2f" % \
        ((read_results['toss_multi_allele_snps'] / total_pairs)*100) + "%)")
...
def write_read_seqs(both_read_seqs, keep, remap_bam, fastqs, dropped=None, remap_num=0):
    ...
    if num_seqs == 0:
        if dropped is not None:
            for read in reads:
                # mark the read as having too many snps so it can be matched
                # to its mate later in the pairing step
                # need to retain all reads for hicpro/mhic pairing to work
                # XD:A:S dropped tag means too many SNPs
                read.set_tag('XD','S',value_type='A')
                dropped.write(read)
            return 0
        else:
            return 0
